...
class GuestEntriesController extends BaseController
{
	...
	private function _returnSuccess($entry)
	{
		...
		{
			craft()->userSession->setNotice(!empty($_POST['successMessage']) ? $_POST['successMessage'] : Craft::t('Entry saved.'));
	...
	private function _returnError($entry)
	{
		...
		{
			craft()->userSession->setError(!empty($_POST['failedMessage']) ? $_POST['failedMessage'] : Craft::t('Couldn’t save entry.'));
